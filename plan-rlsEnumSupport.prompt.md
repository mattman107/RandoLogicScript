...
- 2026-08-08: Replaced target-specific semantic enum categories with first-class named enums. `Type` now retains only generic `Enum` plus cross-game `Region` and `Check`; game-target enum names are declared through `extern enum`. Named enum annotations (for example, `color: Color`) resolve through project metadata and retain enum identity through parameter, call, ternary, and default-value validation. Updated SOH routing, tests, and language documentation.
- 2026-08-08: Removed the `RSK_` and `RO_` prefix fallback and the core `Setting` type. Host setting keys and option values are represented by `extern enum Setting { RSK_*, RO_* }`; `extern define setting(key: Setting) -> Int` returns the setting value. `RR_` and `RC_` remain core fallback prefixes for Region and Check.
